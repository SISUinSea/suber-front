...
- [ ] SearchView :
    ...
    - [x] 유저가 요청할 때 특정 채널의 추가적인 영상을 얻어오는 기능 구현
...
- [x] CartView:
    ...
    - [ ] Cart 에 Select 기능 추가
        ...
        - [ ] 설정한 시간과 영상의 총 합을 비교해주는 UI가 있어야 함
            ...
            - [ ] 설정한 시간보다 크다면 피드백
            - [ ] videos의 동영상은 시청 속도를 조절할 수 있음(default : 1.0x 외에도 1.5, 2.0 배속)
            - [ ] 배속에 맞춰서 동영상 시청 소요 시간도 변화 (예 : 2배속으로 영상을 본다면 영상 길이는 원래 영상의 1/2로 반영됨)
        - Confirm 시
            - 유저가 최종 선택을 한다면, 유튜브 플레이 리스트를 만들고 videos에 있는 영상들을 플레이리스트에 저장.
            - videos의 길이가 설정한 시간보다 많다면 영상의 나머지 시간만큼을 더해서 시청시간을 재설정할 지 물어봐야 함.
                - 만약 시청시간을 재설정 한다면, 시청 시간을 필요한 만큼 늘림
